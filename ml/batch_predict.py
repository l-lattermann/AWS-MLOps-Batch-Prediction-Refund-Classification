...
def finish_run(conn, run_id: str, status: str, processed: int, failed: int, error: str | None = None) -> None:
    with conn.cursor() as cur:
        cur.execute(
            """
...
def main() -> None:
    ...
    with get_connection() as conn, tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            finish_run(conn, run_id, "SUCCESS", processed, failed)
            conn.commit()

        except Exception as exc:
            finish_run(conn, run_id, "FAILED", processed, failed + len(images) - processed - failed, str(exc))
            conn.commit()
            raise

    logger.info("Batch prediction finished | run_id=%s processed=%s failed=%s runtime=%.2fs", run_id, processed, failed, perf_counter() - started)
